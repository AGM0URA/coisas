...
import produtoImagem from "../img/imgProduto.png";
import EditarImagem from "../img/editarEstoque.png";
import imgAdicionarProduto from "../img/maisEstoque.png";
import Pesquisaimg from "../img/lupa.png";

const Estoque = () => {
  const [produtos, setProdutos] = useState([]);
  const [showForm, setShowForm] = useState(false);
  const [novoProduto, setNovoProduto] = useState({ nome: '', quantidade: '', imagem: '' });
    ...
    <ContainerEstoque>
      ...
      <PesquisaEstoque>
        <Input
          ...
          style={{ width: '100%', border: 'none', outline: 'none', borderRadius: '10px', padding: '10px' }}
        />
      ...
      <ProdutosWrapper>
        {produtos
          ...
          .map((produto, index) => (
            <Produto key={index}>
              <ImagemProduto src={produto.imagem || produtoImagem} alt="Imagem do Produto" />
              <ProdutoInfo>
                <TextProduto>{produto.nome}</TextProduto>
                <Textquantidade>Quantidade: {produto.quantidade}</Textquantidade>
              </ProdutoInfo>
              <EditarProduto src={EditarImagem} onClick={() => handleEdit(index)} />
            </Produto>
          ))}
      </ProdutosWrapper>
      {showForm && (
        <FormContainer>
          <form onSubmit={handleSubmit}>
            ...
            <FileInputWrapper>
              ...
              />
              <CustomFileButton onClick={() => document.querySelector('input[type="file"]').click()}>
                Escolher Imagem
              </CustomFileButton>
            </FileInputWrapper>
            {selectedImage && (
              <img src={selectedImage} alt="Imagem Selecionada" style={{ width: '100px', marginTop: '10px' }} />
            )}
            <Button type="submit">{editingIndex !== null ? 'Atualizar Produto' : 'Adicionar Produto'}</Button>
            {editingIndex !== null && (
              <Button type="button" onClick={() => handleDelete(editingIndex)} style={{ backgroundColor: 'red', marginTop: '10px' }}>
                Excluir Produto
              </Button>
            )}
          </form>
